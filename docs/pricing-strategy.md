...
参考 panel 按来源分为两类（处理成本相同，统一定价）：

- **用户提供的 reference panel**：实验室历史文章、历史投稿、用户私有库、客户提供的公开论文或 DOI/PMID 清单。无论内部或外部来源，处理流程一致。
- **PubMed reference panel**：系统按 PubMed 检索式从公开可访问数据中获得的相关论文 panel。额外附加爬虫费用（见 2.4 节）。
...
- 所有 reference panel 处理成本一致（提取、切分、特征生成、索引），统一定价，不按来源区分内部和外部。
- PubMed panel 在统一 reference panel 价格基础上附加爬虫费用（检索、下载、可访问性判断、失败记录），这是唯一的增量成本。
...
| PubMed 爬虫附加 | +¥0.1 / panel | 最多 100 reference panel，仅公开可访问数据 |
...
| target panel | 已有内部库 | 新增 reference panel 额度 | AI 咨询 token | 建议价格 |
| --- | ---: | ---: | ---: | ---: |
| 1-50 | 包含 | ≤1,000 | 3 万（约 10 次） | ¥199 / 篇起 |
| 51-100 | 包含 | ≤1,000 | 5 万（约 16 次） | ¥299 / 篇起 |
| 101-200 | 包含 | ≤2,000 | 8 万（约 26 次） | ¥499 / 篇起 |
| 201-300 | 包含 | ≤2,000 | 12 万（约 40 次） | ¥699 / 篇起 |
| 301-500 | 包含 | ≤4,000 | 20 万（约 66 次） | ¥999 / 篇起 |
| 超过 500 | 定制 | 定制 | 定制 | 先评估 |

上表说明：

- "已有内部库"指实验室已入库的历史文章，查询不额外收费。
- "新增 reference panel"不区分来源（用户提供或 PubMed 获取），处理成本统一。
- PubMed 获取的 panel 额外附加爬虫费用（见 5.2 节）。
- 上表为目标文章基础报告价，不包含超额 reference panel、PubMed 爬虫费和人工专家审核。

### 5.2 reference panel 超额阶梯

所有 reference panel 的处理成本相同（提取、切分、特征生成、索引），不区分来源。仅 PubMed panel 因需要系统检索和下载，附加爬虫费用。

| 超额数量 | reference panel 统一单价 | PubMed 爬虫附加 |
| --- | ---: | ---: |
| 额度内 | 包含 | +¥0.1 / panel |
| 101-500 | ¥0.15 / panel | +¥0.1 / panel |
| 501-2,000 | ¥0.12 / panel | +¥0.08 / panel |
| 2,001-5,000 | ¥0.08 / panel | +¥0.06 / panel |
| 超过 5,000 | 年度 / 定制报价 | 年度 / 定制报价 |

解释：

- 用户提供的 reference panel（无论来自内部历史还是外部参考）处理完全一致，统一定价。
- PubMed 爬虫附加仅覆盖检索、下载、公开可访问性判断和失败记录的增量成本。
- 已入库的内部 panel 已完成索引，查询不产生额外 reference panel 费用。

### 5.3 价格示例

示例 A：个人用户，目标文章 42 target panel，提供 4 篇参考论文共 80 reference panel。  
报价：Personal M ¥199（80 reference panel 在额度 100 内）。

示例 B：实验室用户，目标文章 86 target panel，使用已有内部库，另提供 300 新增 reference panel。  
报价：基础 ¥299（300 reference panel 在额度 1,000 内），总价 ¥299。

示例 C：实验室用户，目标文章 160 target panel，需要 PubMed 补足 1,200 reference panel。  
报价：基础 ¥499，1,200 panel 在额度 2,000 内（处理费包含），PubMed 爬虫 1,200 × ¥0.1 = ¥120，总价 ¥619。

示例 D：复杂文章 420 target panel，用户提供 3,000 reference panel。  
报价：基础 ¥999，3,000 panel 在额度 4,000 内（处理费包含），无 PubMed 爬虫费，总价 ¥999。

## 6. 相关库与 corpus 建库

### 6.1 用户提供的 corpus 建库

用户提供 PDF、figure 或 DOI/PMID 清单的 corpus 建库，不区分内部和外部来源，统一按入库成功的 reference panel 计费。

| 规模 | 年度套餐客户 | 非套餐客户 |
| --- | ---: | ---: |
| ≤20,000 reference panel | 包含在套餐基础额度 | ¥1,500 / corpus（一次性建库费） |
| 20,001-100,000 reference panel | ¥0.08 / panel | ¥0.1 / panel |
| 超过 100,000 reference panel | 定制 | 定制 |

说明：内部私有库是实验室版核心卖点，不建议个人版开放。年度套餐客户的历史库建设已在基础额度内，不额外收费。

### 6.2 PubMed 公开数据 topic

PubMed 建库在统一 reference panel 处理价基础上附加爬虫费用。

| 项目 | 建议价格 | 说明 |
| --- | ---: | --- |
| 检索策略设计 | ¥800 / topic | query、MeSH、关键词、范围确认 |
| reference panel 入库 | 统一价（见 6.1） | 按成功入库 panel 计费 |
| PubMed 爬虫附加 | +¥0.08 / panel | 含检索、下载、可访问性判断、失败记录 |
| 定期刷新 | ¥800 / 次 / topic | 最多 1,000 篇新增候选 |

示例：年度 Starter 客户新建一个 PubMed topic（20,000 panel）：检索策略 ¥800 + 入库处理包含在年度额度 + 爬虫 20,000 × ¥0.08 = ¥1,600，总计 ¥2,400。
...
| reference panel 超额 | ¥0.15 / panel | ¥0.12 / panel | ¥0.08 / panel |
| PubMed 爬虫附加 | +¥0.1 / panel | +¥0.08 / panel | +¥0.06 / panel |
| AI 咨询 token 超额 | ¥0.8 / 万 token | ¥0.5 / 万 token | ¥0.3 / 万 token |
| PubMed topic 检索策略 | ¥800 / topic | ¥600 / topic | ¥500 / topic |
| 完整专家审核 | 9 折 | 85 折 | 8 折 |
